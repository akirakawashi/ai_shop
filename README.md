...
уведомление «Откройте вторую кассу», когда в заданной зоне ожидания непрерывно
более 20 секунд находятся минимум два человека.
...
confirming_full
   ↓ вместимость непрерывно превышена дольше заданного времени
...
EVENT_ROI_CAPACITY=2
EVENT_FULL_CONFIRM_SECONDS=20.0
...
`EVENT_FULL_CONFIRM_SECONDS` измеряется по монотонным часам, а не по количеству
кадров, поэтому пропуски и изменение FPS не сокращают 20-секундное ожидание.
Если количество людей становится меньше `EVENT_ROI_CAPACITY`, таймер
подтверждения сбрасывается.

Вместимость и время ожидания можно изменить независимо:
...
2. Установить вместимость `2` и `EVENT_FULL_CONFIRM_SECONDS=20.0`.
3. Запустить приложение без `--dry-run`.
4. Двум людям войти в ROI и непрерывно оставаться там больше 20 секунд.
5. Проверить одно сообщение и JPEG в Telegram.
6. Остаться внутри и убедиться, что повторных сообщений нет.
7. Выйти минимум на десять кадров, снова войти вдвоём и пробыть более 20 секунд:
   должно прийти новое сообщение с учётом настроенного cooldown.
